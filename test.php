        ...
        switch ($state) {
            case 0:
                if ($this->checkReg($conn, $user_id) == true) {
                    $this->replyToChat(
                        ...
                        "Используйте команду /login, чтобы авторизироваться."
                    );
                    break;
                } else {
            ...
            case 3:
                if ($text === '') {
                    $notes['state'] = 3;
                    $this->conversation->update();
                    $data['text'] = 'Введите свою почту в формате email_name@domen.ru:';
                    $result = Request::sendMessage($data);
                    break;
                }
            ...
            case 4:
                if ($text === '') {
                    $notes['state'] = 4;
                    $this->conversation->update();
                    $data['text'] = 'Введите пароль не короче 8 символов:';
                    $result = Request::sendMessage($data);
                    break;
                }
            ...
            case 5:
                ...
                if ($this->checkPassword($text)) {
                    ...
                    $this->replyToChat("Неверный формат банков карты" . PHP_EOL . "Повторите попытку:");
                    break;
                }
                $text = '';
